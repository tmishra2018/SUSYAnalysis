...
void makeDatacard_SBins_pred(double mGl,double mNLSP,TString sigFile,char histname1[1000],char histname[1000]){
  char name[1000];
  ...
  //  f[1] = new TFile("EWK_LL.root");
  //  f[2] = new TFile("EWK_FR.root");
  //  f[3] = new TFile("EWK_Zin.root");
  f[1] = new TFile("Lostlepton_highdphi.root");
  f[2] = new TFile("Fakerate_highdphi.root");//ZGJetsToNuNuG_v17.root  
  f[3] = new TFile("Zinvi_highdphi.root");
  f[4] = new TFile("QCD_Multi.root");
