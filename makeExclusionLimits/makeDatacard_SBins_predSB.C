...
void makeDatacard_SBins_predSB(double mGl,double mNLSP,TString sigFile,char histname1[1000],char histname[1000]){
  ...
  for(int i=1;i<=imax;i++){
   ...
   //else if(q==0) outf<<getfname(f[3]->GetName())<<"_Bin"<<i<<" gmN "<<hist3[3]->GetBinContent(i)<<" - - - - - - - - "<<Frlow[2]->GetBinContent(i)<<" - "<<endl;
    ...
    else if(i>=41 && i<=46 ) outf<<"st"<<getfname(f[4]->GetName())<<"_Wtag"<<" lnN "<<" - - - - "<<1.0 + (Fr[jmax-1]->GetBinError(i))/(Fr[jmax-1]->GetBinContent(i))<<" - - - - "<<1.0 + (Fr[jmax-1]->GetBinError(i))/(Fr[jmax-1]->GetBinContent(i))<<endl;
