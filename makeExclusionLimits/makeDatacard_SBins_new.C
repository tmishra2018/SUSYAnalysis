...
void makeDatacard_SBins_new(double mGl,double mNLSP,TString sigFile,char histname1[1000],char histname[1000]){
  char name[1000];
  ...
  f[1] = new TFile("WGJetsToLNuG_2016_v17.root");
  f[2] = new TFile("WJetsToLNu_2016_v17.root");
  f[3] = new TFile("TTJetsHT_2016_v17.root");
  f[4] = new TFile("TTGJets_2016_v17.root");
  //  f[5] = new TFile("ZGJetsToNuNuG_v12.root");
  f[5] = new TFile("ZGZJ_NuNuG_2016_v17.root");
  //f[5] = new TFile("ZGJetsToNuNuG_v17.root");
  //f[6] = new TFile("ZJetsToNuNu_v17.root");
  f[6] = new TFile("GJetsQCD_2016_v17.root");
  ...
  //  double scaleToLumi = 35.9/137.19;//137.0;
  double scaleToLumi = 1.0;//137.0;
  ofstream outf;
  vector<int> observation;
  for(int i=0;i<nFiles;i++){
    hist1[i] = (TH1D*)f[i]->FindObjectAny(histname);
    //setLastBinAsOverFlow(hist1[i]);
    if(scaleToLumi < 0.99999 || scaleToLumi > 1.00001){
      cout<<"!!! changing lumi (scaling) to a different value. Scale: "<<scaleToLumi<<endl;
      hist1[i]->Scale(scaleToLumi);
    }
  }
