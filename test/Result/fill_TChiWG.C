...
//	double xserror1(0);
//	if(doC1C1){
//		if(C1C1_file.is_open()){
//  		for(int i(0); i<60; i++){ 
//				C1C1_file >> susymass1 >> xsvalue1 >> xserror1;
//				xsmap.insert(make_pair(susymass1, xsvalue1));
//	  	}
//		}
//	}
//	else{
//		if(C1N1_file.is_open()){
//  		for(int i(0); i<60; i++){ 
...
//	C1C1_file.close();
	...
	if(C1N1_file.is_open() && C1C1_file.is_open()){
      ...
      if(doC1C1)xsmap.insert(make_pair(susymass1, xsvalue2));
			else if(doCIN1)xsmap.insert(make_pair(susymass1, xsvalue1));
			else xsmap.insert(make_pair(susymass1, xsvalue1+xsvalue2));
	  }
	}
	C1N1_file.close();
	C1C1_file.close();
  ...
  datachain->Draw("nVertex >> p_PU_data");
	...
	float Mass2 = 0.5;
	init_histo("TChiWG","test_TChiWG_C1C1.root",2, 48,287.5,1487.5,1,0,1); 

	std::ostringstream histname;
	//**************   T5WG  ***************************//
	TFile *file = TFile::Open("/uscms_data/d3/mengleis/FullStatusOct/resTree_TChiWG.root");
  ...
  tree->SetBranchAddress("nVertex",    &nVertex);
	tree->SetBranchAddress("branch",     &decaybranch);
	...
	for(unsigned ievt(0); ievt < tree->GetEntries(); ievt++){
		...
		if(decaybranch == 1)p_SUSYGEN[1]->Fill(Mass1, Mass2); 
		else if(decaybranch == 2)p_SUSYGEN[2]->Fill(Mass1, Mass2);
		else if(decaybranch == 3)p_SUSYGEN[3]->Fill(Mass1, Mass2);
  ...
  mgtree->SetBranchAddress("ISRPt",      &ISRPt_mg);
	...
	mgtree->SetBranchAddress("branch",     &decaybranch_mg);
    ...
    double s_trg_error =objectESF.getFastMuonEGTRGESFError(phoEt_mg, lepPt_mg)*objectESF.getPhotonESF(phoEt_mg, phoEta_mg);
		...
		if(sigMT_mg > 100){
			...
			if(genMETBinIndex >=0)susy_chan_rate_genMET[genMETBinIndex]->Fill(Mass1_mg, Mass2);
		}	

		int SigBinIndex(-1);
		if(sigMET_mg >= 120 && sigMT_mg >= 100){
			SigBinIndex = Bin.findSignalBin(sigMET_mg, HT_mg, phoEt_mg);
			if(SigBinIndex >= 0)SigBinIndex += NBIN;
		}
		int jesupBinIndex(-1);
		if(sigMETJESup_mg >= 120 && sigMTJESup_mg >= 100){	
			jesupBinIndex = Bin.findSignalBin(sigMETJESup_mg, HTJESup_mg, phoEt_mg);
			if(jesupBinIndex >= 0)jesupBinIndex += NBIN;
		} 
		int jesdoBinIndex(-1);
		if(sigMETJESdo_mg >=120  && sigMTJESdo_mg >= 100){	
			jesdoBinIndex = Bin.findSignalBin(sigMETJESdo_mg, HTJESdo_mg, phoEt_mg);
			if(jesdoBinIndex >= 0)jesdoBinIndex += NBIN;
		}

		if(Mass1_mg > 0 && Mass2 > 0){ 

			if(SigBinIndex >=0){
				susy_chan_rate_nom[0][SigBinIndex]->Fill( Mass1_mg, Mass2, scalefactor);
				susy_chan_rate_control[0][SigBinIndex]->Fill( Mass1_mg, Mass2);
	
				if(decaybranch_mg == 1){
					susy_chan_rate_nom[1][SigBinIndex]->Fill( Mass1_mg, Mass2, scalefactor);
					susy_chan_rate_control[1][SigBinIndex]->Fill( Mass1_mg, Mass2);
				}
				else if(decaybranch_mg == 2){
					susy_chan_rate_nom[2][SigBinIndex]->Fill( Mass1_mg, Mass2, scalefactor);
					susy_chan_rate_control[2][SigBinIndex]->Fill( Mass1_mg, Mass2);
				}
				else if(decaybranch_mg == 3){
					susy_chan_rate_nom[3][SigBinIndex]->Fill( Mass1_mg, Mass2, scalefactor);
					susy_chan_rate_control[3][SigBinIndex]->Fill( Mass1_mg, Mass2);
				}

				susy_chan_rate_phoesfUp[SigBinIndex]->Fill( Mass1_mg, Mass2, phoESFUp*lepESF); 
				susy_chan_rate_lepesfUp[SigBinIndex]->Fill( Mass1_mg, Mass2, phoESF*lepESFUp); 
				susy_chan_rate_isr[SigBinIndex]->Fill(Mass1_mg, Mass2, scalefactor*reweightF);	
	
				if(nVertex_mg < 20)p_lowPU_pass->Fill( Mass1_mg, Mass2, 1);
				else p_highPU_pass->Fill( Mass1_mg, Mass2, 1);

			  for(unsigned is(0); is < 9; is++)
					susy_chan_rate_pdf[SigBinIndex][is]->Fill(Mass1_mg, Mass2, scalefactor*(*ScaleSystWeight_mg)[is]);
			}
			if(jesupBinIndex >=0){
				susy_chan_rate_jesUp[jesupBinIndex]->Fill( Mass1_mg, Mass2, scalefactor); 
			}
			if(jesdoBinIndex >=0){
				susy_chan_rate_jesDown[jesdoBinIndex]->Fill( Mass1_mg, Mass2, scalefactor);   
			}	
		}
	}
  ...
  egtree->SetBranchAddress("ISRPt",      &ISRPt_eg);
	...
	egtree->SetBranchAddress("branch",     &decaybranch_eg);
	...
	for(unsigned ievt(0); ievt < egtree->GetEntries(); ievt++){
		...
		if(sigMT_eg > 100){
			...
			if(genMETBinIndex >=0)susy_chan_rate_genMET[genMETBinIndex]->Fill(Mass1_eg, Mass2);
		}	

		int SigBinIndex(-1);
		if(sigMET_eg >= 120 && sigMT_eg >= 100){
			SigBinIndex = Bin.findSignalBin(sigMET_eg, HT_eg, phoEt_eg);
		}
		int jesupBinIndex(-1);	
		if(sigMETJESup_eg >= 120 && sigMTJESup_eg >= 100){
			jesupBinIndex = Bin.findSignalBin(sigMETJESup_eg, HTJESup_eg, phoEt_eg);
		}
		int jesdoBinIndex(-1);
		if(sigMETJESdo_eg >= 120 && sigMTJESdo_eg >= 100){
			jesdoBinIndex = Bin.findSignalBin(sigMETJESdo_eg, HTJESdo_eg, phoEt_eg);
		}

		if(Mass1_eg > 0 && Mass2 > 0){ 

			if(SigBinIndex >=0){
				susy_chan_rate_nom[0][SigBinIndex]->Fill( Mass1_eg, Mass2, scalefactor);
				susy_chan_rate_control[0][SigBinIndex]->Fill( Mass1_eg, Mass2);
	
				if(decaybranch_eg == 1){
					susy_chan_rate_nom[1][SigBinIndex]->Fill( Mass1_eg, Mass2, scalefactor);
					susy_chan_rate_control[1][SigBinIndex]->Fill( Mass1_eg, Mass2);
				}
				else if(decaybranch_eg == 2){
					susy_chan_rate_nom[2][SigBinIndex]->Fill( Mass1_eg, Mass2, scalefactor);
					susy_chan_rate_control[2][SigBinIndex]->Fill( Mass1_eg, Mass2);
				}
				else if(decaybranch_eg == 3){
					susy_chan_rate_nom[3][SigBinIndex]->Fill( Mass1_eg, Mass2, scalefactor);
					susy_chan_rate_control[3][SigBinIndex]->Fill( Mass1_eg, Mass2);
				}

				susy_chan_rate_phoesfUp[SigBinIndex]->Fill( Mass1_eg, Mass2, phoESFUp*lepESF); 
				susy_chan_rate_lepesfUp[SigBinIndex]->Fill( Mass1_eg, Mass2, phoESF*lepESFUp); 
				susy_chan_rate_isr[SigBinIndex]->Fill(Mass1_eg, Mass2, scalefactor*reweightF);	
	
				if(nVertex_eg < 20)p_lowPU_pass->Fill( Mass1_eg, Mass2, 1);
				else p_highPU_pass->Fill( Mass1_eg, Mass2, 1);

			  for(unsigned is(0); is < 9; is++)
					susy_chan_rate_pdf[SigBinIndex][is]->Fill(Mass1_eg, Mass2, scalefactor*(*ScaleSystWeight_eg)[is]);
			}
			if(jesupBinIndex >=0){
				susy_chan_rate_jesUp[jesupBinIndex]->Fill( Mass1_eg, Mass2, scalefactor); 
			}
			if(jesdoBinIndex >=0){
				susy_chan_rate_jesDown[jesdoBinIndex]->Fill( Mass1_eg, Mass2, scalefactor);   
			}	
		}
	}
...
//************ Normalize Hists **************** //
	...
	for(unsigned i(1); i < p_SUSYMass->GetXaxis()->GetNbins() + 1; i++){
		unsigned j = 1;
			...
			for(unsigned ih(0); ih < NBIN*2; ih++){
				...
				int sparticleMass1 = int(p_SUSYMass->GetXaxis()->GetBinCenter(i));
          ...
          susy_chan_rate_nom[ich][ih]->Sumw2();	
	...
	for(unsigned i(1); i < p_SUSYMass->GetXaxis()->GetNbins() + 1; i++){
		unsigned j = 1;
			...
			int totalevt(0);
			double ave_jes(0), ave_phoesf(0), ave_lepesf(0), ave_isr(0), ave_met(0); 	
			for(unsigned ih(0); ih < NBIN*2; ih++){
			...
			for(unsigned ih(0); ih < NBIN*2; ih++){
				if(susy_chan_rate_control[0][ih]->GetBinContent(i,j) == 0){
					...
					susy_chan_syserr_pdf[ih]->SetBinContent(i,j, -1); 
