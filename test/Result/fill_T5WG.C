...
void fill_T5WG(){//main  
  ...
  gROOT->SetBatch();
	...
	TH1D *p_crosssection = (TH1D*)xSecFile.Get("p_gluinoxSec");
  ...
  datachain->Draw("nVertex >> p_PU_data");
	...
	init_histo("T5WG","/uscms_data/d3/tmishra/test_T5WG_ext.root",2,36, 775.0, 2575.0, 500, 2.5, 2502.5);
	std::ostringstream histname;
	//**************   T5WG  ***************************//
	TFile *file = TFile::Open("/uscms_data/d3/mengleis/FullStatusOct/resTree_T5WG_ext.root");
		...
		for(int j(1); j < p_SUSYMass->GetYaxis()->GetNbins() + 1; j++){
			...
			for(unsigned ih(0); ih < NBIN*2; ih++){
				...
				for(unsigned ich(0); ich < 4; ich++){	
					...
					susy_chan_rate_nom[ich][ih]->Sumw2(kFALSE);
					susy_chan_rate_nom[ich][ih]->SetBinContent(i,j, susy_chan_rate_nom[ich][ih]->GetBinContent(i,j)*35.9*crosssection/noe_ch);
					susy_chan_rate_nom[ich][ih]->SetBinError(i,j, susy_chan_rate_nom[ich][ih]->GetBinError(i,j)*35.9*crosssection/noe_ch);	
				}
			}
		}
	} 
