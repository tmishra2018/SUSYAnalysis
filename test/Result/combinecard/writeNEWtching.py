...
outdir = '/uscmst1b_scratch/lpc1/3DayLifetime/mengleis/TChiNG/'

susy_in = ROOT.TFile('../test_TChiNG.root','read')
n_channels = 36
...
for i in range(1, n_channels + 1):
    ...
    h_rates['WW_chan' + str(i) + '_rate_nom'] = susy_in.Get('WW_chan' + str(i) + '_rate_nom')
    h_rates['WG_chan' + str(i) + '_rate_control'] = susy_in.Get('WG_chan' + str(i) + '_rate_control')
    h_rates['GG_chan' + str(i) + '_rate_control'] = susy_in.Get('GG_chan' + str(i) + '_rate_control')
    h_rates['WW_chan' + str(i) + '_rate_control'] = susy_in.Get('WW_chan' + str(i) + '_rate_control')
    for j in syst_names:
        ...
        for l in lines:
            if (re.search('NSC', l)
                ...
                for k in range(1, n_channels+1):
                    ...
                    n_gz  = h_rates['WG_chan' + str(k) + '_rate_nom'].GetBinContent(i, j)
                    con_gz= h_rates['WG_chan' + str(k) + '_rate_control'].GetBinContent(i, j)
                    n_zz  = h_rates['WW_chan' + str(k) + '_rate_nom'].GetBinContent(i, j)
                    con_zz= h_rates['WW_chan' + str(k) + '_rate_control'].GetBinContent(i, j) 
                    n_nom = n_gg*br_gg + n_gz*br_gz + n_zz*br_zz 
                    con_nom = con_gg + con_gz + con_zz 
