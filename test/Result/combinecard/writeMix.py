...
outdir = '/uscmst1b_scratch/lpc1/3DayLifetime/mengleis/TChiNGWG/'

tching_in = ROOT.TFile('../test_TChiNG_C1C1.root','read')
tchiwg_in = ROOT.TFile('../test_TChiWG_C1C1.root','read')

n_channels = 36
syst_names = ['jes','phoesf','lepesf','lumi','isr','pdf','met']

h_SUSYmass = tching_in.Get('SUSYMass')
h_rates = {}
for i in range(1, n_channels + 1):
    h_rates['ALL_chan' + str(i) + '_rate_nom'] = tchiwg_in.Get('ALL_chan' + str(i) + '_rate_nom')
    h_rates['ALL_chan' + str(i) + '_rate_control'] = tchiwg_in.Get('ALL_chan' + str(i) + '_rate_control')
    h_rates['GG_chan' + str(i) + '_rate_nom'] = tching_in.Get('GG_chan' + str(i) + '_rate_nom')
    h_rates['GG_chan' + str(i) + '_rate_control'] = tching_in.Get('GG_chan' + str(i) + '_rate_control')
    for j in syst_names:
        h_rates['susy_chan' + str(i) + '_syserr_' + j] = tching_in.Get('susy_chan' + str(i) + '_syserr_' + j)
h_rates['susy_syserr_PU'] = tching_in.Get('susy_syserr_PU')
...
for i in range(1, h_SUSYmass.GetXaxis().GetNbins() + 1):
    ...
    for br in range(0,51):
        ...
        file_out = open(
            outdir + 'counting_tchiNg_'
            + str(int(h_SUSYmass.GetXaxis().GetBinCenter(i)))
            + '_BF_' + str(int(br*2)) +  '.txt', 'w') 
        ...
        for l in lines:
            if (re.search('NSC', l)
                ...
                for k in range(1, n_channels+1):
                    ...
                    n_gg  = h_rates['GG_chan' + str(k) + '_rate_nom'].GetBinContent(i, j)
                    con_gg= h_rates['GG_chan' + str(k) + '_rate_control'].GetBinContent(i, j)
                    n_gz  = h_rates['ALL_chan' + str(k) + '_rate_nom'].GetBinContent(i, j)
                    con_gz= h_rates['ALL_chan' + str(k) + '_rate_control'].GetBinContent(i, j)
                    n_nom = n_gg*br_gg + n_gz*br_gz
                    con_nom = con_gg + con_gz 
                    unc_jes = 1+ h_rates['susy_chan' + str(k) + '_syserr_jes'].GetBinContent(i, j)
                    ...
                    unc_pdf =  1+ h_rates['susy_chan' + str(k) + '_syserr_pdf'].GetBinContent(i, j)
                    ...
                    unc_stat = 1.0
                    if n_nom != 0.0:
                        unc_stat = 1 + math.sqrt(con_nom)/con_nom 
                    else:
                        unc_stat = 2.0
                    ...
                    if re.search('NSC', l):
                        ...
                        l = l.replace('PDFSCALE' + str(binn)+ ' ', str(round(unc_pdf,3))) 
