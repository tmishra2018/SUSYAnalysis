...
susy_in = ROOT.TFile('/uscms_data/d3/tmishra/test_T5WG_ext.root','read')
n_channels = 36
...
for i in range(1, h_SUSYmass.GetXaxis().GetNbins() + 1):
    for j in range(1, h_SUSYmass.GetYaxis().GetNbins() + 1):
        ...
        for br in range(25,26):
          ...
          br_zz = 0 
          file_out = open(
            '/uscmst1b_scratch/lpc1/3DayLifetime/Tribeni/T5WG_ext/counting_t5_'
            + str(int(h_SUSYmass.GetXaxis().GetBinCenter(i))) + '_'
            + str(int(h_SUSYmass.GetYaxis().GetBinCenter(j))) + '_BF_' + str(int(br*2)) + '.txt', 'w'
            )
