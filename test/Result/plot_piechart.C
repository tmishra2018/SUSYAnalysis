...
void plot_piechart(int NBIN){//main  
	gROOT->SetBatch();
	gStyle->SetOptStat(0);
	setTDRStyle();
	gStyle->SetErrorX(0);
  gSystem->Load("../../lib/libAnaClasses.so");
        ...
        TFile *egfile_ele = TFile::Open("/uscms_data/d3/tmishra/Output/signalTree_egamma_eleBkg.root");
        TFile *egfile_jet = TFile::Open("/uscms_data/d3/tmishra/Output/signalTree_egamma_jetbkg.root");
        TFile *egfile_qcd = TFile::Open("/uscms_data/d3/tmishra/Output/signalTree_egamma_qcd.root");
        TFile *egfile_VG  = TFile::Open("/uscms_data/d3/tmishra/Output/signalTree_egamma_VGBkg.root");
        TFile *egfile_rare= TFile::Open("/uscms_data/d3/tmishra/Output/signalTree_egamma_rareBkg.root");

        TFile *mgfile_sig = TFile::Open("/uscms_data/d3/tmishra/Output/signalTree_mg_signal.root");
        TFile *egfile_sig = TFile::Open("/uscms_data/d3/tmishra/Output/signalTree_egamma_signal.root");
  ...
  TGraphAsymmErrors* ratio = new TGraphAsymmErrors();	
	...
	double bkgqcdfakelep = 0, bkgelefakepho = 0, bkgjetfakepho = 0, bkgVGamma = 0, bkgRare = 0;
	...
	for(int ibin(1); ibin <= NBIN; ibin++){
	//for(int ibin(NBIN); ibin <= 2*NBIN; ibin++){
	//for(int ibin(1); ibin <= 2*NBIN; ibin++){
		//bkgContent=0; 
		bkgError = 0;
		bkgContent = bkgContent + h_elefakepho_norm->GetBinContent(ibin) + h_jetfakepho_norm->GetBinContent(ibin) + h_qcdfakelep_norm->GetBinContent(ibin) +
								 h_VGamma_norm->GetBinContent(ibin)     + h_rare_norm->GetBinContent(ibin);

		bkgqcdfakelep = bkgqcdfakelep + h_qcdfakelep_norm->GetBinContent(ibin);
		bkgelefakepho = bkgelefakepho + h_elefakepho_norm->GetBinContent(ibin);
		bkgjetfakepho = bkgjetfakepho + h_jetfakepho_norm->GetBinContent(ibin);
		bkgVGamma = bkgVGamma + h_VGamma_norm->GetBinContent(ibin);
		bkgRare = bkgRare + h_rare_norm->GetBinContent(ibin);
		...
		//cout<<ibin <<"\t"<<bkgContent<<"\t"<<bkgError<<"\t"<<sqrt(bkgContent)<<endl;
		...
		cout<<"VGamma  "<<bkgVGamma/bkgContent <<endl;
		cout<<"elefakepho  "<<bkgelefakepho/bkgContent <<endl;
		cout<<"jetfakepho  "<<bkgjetfakepho/bkgContent <<endl;
		cout<<"qcdfakelep  "<<bkgqcdfakelep/bkgContent <<endl;
		cout<<"Rare  "<<bkgRare/bkgContent <<endl;
