...
void fill_M1M2(){//main  
  ...
  gSystem->Load("/uscms/home/mengleis/work/SUSY2016/SUSYAnalysis/lib/libAnaClasses.so");
	...
	std::ifstream M1M2_file("../cross/CrossSectionM1M2.txt");
  ...
  datachain->Draw("nVertex >> p_PU_data");
	...
	init_histo("M1M2","test_M1M2.root",2, 27,175,1525,26,225,1525);

	std::ostringstream histname;
	//**************   T5WG  ***************************//
	TFile *file = TFile::Open("/uscms_data/d3/mengleis/FullStatusOct/resTree_GMSB_M1M2.root");
  ...
  tree->SetBranchAddress("nVertex",    &nVertex);
	...
	for(unsigned ievt(0); ievt < tree->GetEntries(); ievt++){
		...
		if(decaybranch == 2)p_SUSYGEN[1]->Fill(Mass1, Mass2); 
		else if(decaybranch == 1)p_SUSYGEN[2]->Fill(Mass1, Mass2);
