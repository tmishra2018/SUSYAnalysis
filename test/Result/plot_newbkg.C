...
// Errors
 ...
 binnumber = p_rareHT->GetSize()-2;    p_rareHT->SetBinError(binnumber,    p_rareHT->GetBinError(binnumber) +     p_rareHT->GetBinError(binnumber+1) );                  
	...
	if(channel == 1){
			etplot << "NEWSIGNAL_egamma_2016ReMiniAOD_pt.pdf";     
			ptplot << "NEWSIGNAL_egamma_2016ReMiniAOD_leppt.pdf";	 
			metplot << "NEWSIGNAL_egamma_2016ReMiniAOD_met.pdf";   
			mtplot << "NEWSIGNAL_egamma_2016ReMiniAOD_mt.pdf";     
			htplot << "NEWSIGNAL_egamma_2016ReMiniAOD_ht.pdf";	   
	}                                                      
	else if(channel == 2){
			etplot << "NEWSIGNAL_mg_2016ReMiniAOD_pt.pdf";
			ptplot << "NEWSIGNAL_mg_2016ReMiniAOD_leppt.pdf";		
			metplot << "NEWSIGNAL_mg_2016ReMiniAOD_met.pdf";
			mtplot << "NEWSIGNAL_mg_2016ReMiniAOD_mt.pdf";
			htplot << "NEWSIGNAL_mg_2016ReMiniAOD_ht.pdf";	
	}
	// ******** Mt ************************//
	gStyle->SetOptStat(0);
...
//		prederror += p_rareMt->GetBinError(ibin);
  ...
  chantex.DrawLatex(0.4,0.85,"e#gamma, M_{T} > 100 GeV, p_{T}^{miss} > 120 GeV");
	leg_mt->Draw("same");
  CMS_lumi( pt_pad1, 11 );
 	gPad->RedrawAxis();
...
// ******** MET ************************//
  ...
  chantex.SetTextSize(0.05);    
  chantex.DrawLatex(0.4,0.85,"e#gamma, M_{T} > 100 GeV");
 	gPad->RedrawAxis();
...
// ******** HT ************************//
	...
	p_allHT->SetMaximum(100*p_allHT->GetBinContent(p_allHT->GetMaximumBin()));
