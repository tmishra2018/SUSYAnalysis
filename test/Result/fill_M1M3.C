...
void fill_M1M3(){//main  
  ...
  gSystem->Load("/uscms/home/mengleis/work/SUSY2016/SUSYAnalysis/lib/libAnaClasses.so");
	...
	std::ifstream M1M3_file("../cross/CrossSectionM1M3.txt");
	...
	double xsvalue(0);
	double xserror(0);
	if(M1M3_file.is_open()){
  	for(int i(0); i<930; i++){ 
			M1M3_file >> susymass1 >> susymass2 >>  xsvalue >> xserror;
  ...
  datachain->Draw("nVertex >> p_PU_data");
	...
	init_histo("M1M3","test_M1M3.root",2, 30,25,1525,31,975,2525);

	std::ostringstream histname;
	//**************   T5WG  ***************************//
	TFile *file = TFile::Open("/uscms_data/d3/mengleis/FullStatusOct/resTree_GMSB_M1M3.root");
	...
	for(unsigned i(1); i < p_SUSYMass->GetXaxis()->GetNbins() + 1; i++){
		for(unsigned j(1); j < p_SUSYMass->GetYaxis()->GetNbins() + 1; j++){
			...
			for(unsigned ih(0); ih < NBIN*2; ih++){
				...
				int sparticleMass2 = int(p_SUSYMass->GetYaxis()->GetBinCenter(j));
				...
				if(max( err_jesUp, err_jesDo)/nom_yield > 0.1)std::cout << "jes " << nom_yield << " " << err_jesUp << " " << err_jesDo << std::endl; 
