...
void pred_WZG(){
	...
	if(channelType==1)outputname << "egamma_wzgBkg";
	else if(channelType==2)outputname << "mg_wzgBkg";
	...
	TH1D *h_wzg_norm = 0;
	TH1D *h_wzg_control = 0;
	TH1D *h_wzg_jesUp = 0;
	TH1D *h_wzg_jesDown = 0;
	TH1D *h_wzg_jerUp = 0;
	TH1D *h_wzg_jerDown = 0;
	TH1D *h_wzg_esfUp = 0;
	TH1D *h_wzg_syserr_jes = 0;
	TH1D *h_wzg_syserr_jer = 0;
	TH1D *h_wzg_syserr_esf = 0;
	TH1D *h_wzg_syserr_scale = 0;
	TH1D *h_wzg_syserr_eleshape = 0;
	TH1D *h_wzg_syserr_jetshape = 0;
	TH1D *h_wzg_syserr_qcdshape = 0;
	TH1D *h_wzg_syserr_xs = 0;
	TH1D *h_wzg_syserr_lumi = 0;
	TH1D *h_wzg_syserr_isr = 0;
	if(channelType==1){
		h_wzg_norm            = new TH1D("eg_wzg_norm","eventcount",NBIN,0,NBIN);
		h_wzg_control         = new TH1D("eg_wzg_control","eventcount",NBIN,0,NBIN);
		h_wzg_jesUp           = new TH1D("eg_wzg_jesUp","eventcount",NBIN,0,NBIN);
		h_wzg_jesDown         = new TH1D("eg_wzg_jesDown","eventcount",NBIN,0,NBIN);
		h_wzg_jerUp           = new TH1D("eg_wzg_jerUp","eventcount",NBIN,0,NBIN);
		h_wzg_jerDown         = new TH1D("eg_wzg_jerDown","eventcount",NBIN,0,NBIN);
		h_wzg_esfUp           = new TH1D("eg_wzg_esfUp","eventcount",NBIN,0,NBIN);
		h_wzg_syserr_jes      = new TH1D("eg_wzg_syserr_jes","",NBIN,0,NBIN);	
		h_wzg_syserr_jer      = new TH1D("eg_wzg_syserr_jer","",NBIN,0,NBIN);	
		h_wzg_syserr_esf      = new TH1D("eg_wzg_syserr_esf","",NBIN,0,NBIN);	
		h_wzg_syserr_scale    = new TH1D("eg_wzg_syserr_scale","",NBIN,0,NBIN);	
		h_wzg_syserr_eleshape = new TH1D("eg_wzg_syserr_eleshape","",NBIN,0,NBIN);	
		h_wzg_syserr_jetshape = new TH1D("eg_wzg_syserr_jetshape","",NBIN,0,NBIN);	
		h_wzg_syserr_qcdshape = new TH1D("eg_wzg_syserr_qcdshape","",NBIN,0,NBIN);	
		h_wzg_syserr_xs       = new TH1D("eg_wzg_syserr_xs","",NBIN,0,NBIN);	
		h_wzg_syserr_lumi     = new TH1D("eg_wzg_syserr_lumi","",NBIN,0,NBIN);
		h_wzg_syserr_isr      = new TH1D("eg_wzg_syserr_isr","",NBIN,0,NBIN);
	} 
	else if(channelType==2){
		h_wzg_norm            = new TH1D("mg_wzg_norm","eventcount",NBIN,0,NBIN);
		h_wzg_control         = new TH1D("mg_wzg_control","eventcount",NBIN,0,NBIN);
		h_wzg_jesUp           = new TH1D("mg_wzg_jesUp","eventcount",NBIN,0,NBIN);
		h_wzg_jesDown         = new TH1D("mg_wzg_jesDown","eventcount",NBIN,0,NBIN);
		h_wzg_jerUp           = new TH1D("mg_wzg_jerUp","eventcount",NBIN,0,NBIN);
		h_wzg_jerDown         = new TH1D("mg_wzg_jerDown","eventcount",NBIN,0,NBIN);
		h_wzg_esfUp           = new TH1D("mg_wzg_esfUp","eventcount",NBIN,0,NBIN);
		h_wzg_syserr_jes      = new TH1D("mg_wzg_syserr_jes","",NBIN,0,NBIN);	
		h_wzg_syserr_jer      = new TH1D("mg_wzg_syserr_jer","",NBIN,0,NBIN);	
		h_wzg_syserr_esf      = new TH1D("mg_wzg_syserr_esf","",NBIN,0,NBIN);	
		h_wzg_syserr_scale    = new TH1D("mg_wzg_syserr_scale","",NBIN,0,NBIN);	
		h_wzg_syserr_eleshape = new TH1D("mg_wzg_syserr_eleshape","",NBIN,0,NBIN);	
		h_wzg_syserr_jetshape = new TH1D("mg_wzg_syserr_jetshape","",NBIN,0,NBIN);	
		h_wzg_syserr_qcdshape = new TH1D("mg_wzg_syserr_qcdshape","",NBIN,0,NBIN);	
		h_wzg_syserr_xs       = new TH1D("mg_wzg_syserr_xs","",NBIN,0,NBIN);	
		h_wzg_syserr_lumi     = new TH1D("mg_wzg_syserr_lumi","",NBIN,0,NBIN);
		h_wzg_syserr_isr      = new TH1D("mg_wzg_syserr_isr","",NBIN,0,NBIN);
	} 
	...
	for(unsigned ievt(0); ievt < mctree->GetEntries(); ievt++){
		...
		if(sigMT >= lowMt && (highMt < 0 || (highMt > 0 && sigMT < highMt)) ){
			...
			if(sigMET >= lowMET && (highMET < 0|| (highMET > 0 && sigMET < highMET))){
				...
				if( SigBinIndex >= 0){ 
					  h_wzg_norm->Fill( SigBinIndex, weight);
					  h_wzg_control->Fill( SigBinIndex);
					  h_wzg_esfUp->Fill( SigBinIndex, weight_scaleup);
				}
		...
		if(sigMTJESup >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJESup < highMt)) ){
			if(sigMETJESup >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJESup < highMET))){
				if(Bin.findSignalBin(sigMETJESup, HTJESup, phoEt)>=0)h_wzg_jesUp->Fill( Bin.findSignalBin(sigMETJESup, HTJESup, phoEt),  weight);
		...
		if(sigMTJESdo >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJESdo < highMt)) ){
			if(sigMETJESdo >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJESdo < highMET))){
				if(Bin.findSignalBin(sigMETJESdo, HTJESdo, phoEt)>=0)h_wzg_jesDown->Fill( Bin.findSignalBin(sigMETJESdo, HTJESdo, phoEt),  weight);
		...
		if(sigMTJERup >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJERup < highMt)) ){
			if(sigMETJERup >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJERup < highMET))){
				if(Bin.findSignalBin(sigMETJERup, HT, phoEt)>=0)h_wzg_jerUp->Fill( Bin.findSignalBin(sigMETJERup, HT, phoEt),       weight);
		...
		if(sigMTJERdo >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJERdo < highMt)) ){
			if(sigMETJERdo >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJERdo < highMET))){
				if(Bin.findSignalBin(sigMETJERdo, HT, phoEt)>=0)h_wzg_jerDown->Fill( Bin.findSignalBin(sigMETJERdo, HT, phoEt),       weight); 
	...
	for(int contbin(1); contbin <=NBIN; contbin++){
		binSum= binSum+h_wzg_norm->GetBinContent(contbin);
		float nominalsig = h_wzg_norm->GetBinContent(contbin); 
	//	std::cout << " bin " << contbin << " wzg " << nominalsig << " error " << h_wzg_norm->GetBinError(contbin) << " rel error " << h_wzg_norm->GetBinError(contbin)/h_wzg_norm->GetBinContent(contbin) << " rel error in control region " << sqrt(h_wzg_control->GetBinContent(contbin))/h_wzg_control->GetBinContent(contbin) << std::endl;
		float jesuperror = fabs(h_wzg_jesUp->GetBinContent(contbin)- nominalsig);
		float jesdoerror = fabs(h_wzg_jesDown->GetBinContent(contbin)- nominalsig);
		float jeruperror = fabs(h_wzg_jerUp->GetBinContent(contbin)- nominalsig);
		float jerdoerror = fabs(h_wzg_jerDown->GetBinContent(contbin)- nominalsig);
		float esfuperror = fabs(h_wzg_esfUp->GetBinContent(contbin)- nominalsig); 	
	//	std::cout << " jesup " << jesuperror << " jesdo " << jesdoerror << " jerup " << jeruperror << " jerdo " << jerdoerror << std::endl;

		h_wzg_syserr_jes->SetBinContent(contbin, max(jesuperror,jesdoerror));
		h_wzg_syserr_jer->SetBinContent(contbin, max(jeruperror,jerdoerror));
		h_wzg_syserr_esf->SetBinContent(contbin, esfuperror);
		h_wzg_syserr_scale->SetBinContent(contbin, -1);
		h_wzg_syserr_eleshape->SetBinContent(contbin, -1);
		h_wzg_syserr_jetshape->SetBinContent(contbin, -1);
		h_wzg_syserr_qcdshape->SetBinContent(contbin, -1);
		h_wzg_syserr_xs->SetBinContent(contbin, 0.5*h_wzg_norm->GetBinContent(contbin));     
		h_wzg_syserr_lumi->SetBinContent(contbin, 0.026*h_wzg_norm->GetBinContent(contbin));      
		h_wzg_syserr_isr->SetBinContent(contbin, -1);      
	}
