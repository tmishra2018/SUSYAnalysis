...
void pred_rareBkg_debugging(){
	...
	TH1F* h_rare_unweighted = new TH1F("h_rare_unweighted", "Rare Nominal (Unweighted)", NBIN, 0, NBIN);
	...
	TH1D *scaleup_dPhiEleMET = new TH1D("scaleup_dPhiEleMET","dPhiEleMET",32,0,3.2); 
	        ...
	        TH1F* h_rare_jesUp_nomstay   = (TH1F*) h_rare_norm->Clone("h_rare_jesUp_nomstay");
        TH1F* h_rare_jesDown_nomstay = (TH1F*) h_rare_norm->Clone("h_rare_jesDown_nomstay");
        TH1F* h_rare_jerUp_nomstay   = (TH1F*) h_rare_norm->Clone("h_rare_jerUp_nomstay");
        TH1F* h_rare_jerDown_nomstay = (TH1F*) h_rare_norm->Clone("h_rare_jerDown_nomstay");

        h_rare_jesUp_nomstay->Reset();
        h_rare_jesDown_nomstay->Reset();
        h_rare_jerUp_nomstay->Reset();
        h_rare_jerDown_nomstay->Reset();
...
//	mctree->Add(Form("/eos/uscms/store/user/tmishra/VGamma/resTree_VGamma_TTJets_%d%s.root",RunYear,whichVFP.c_str())); // 25% more
	mctree->Add(Form("/eos/uscms/store/user/tmishra/VGamma/resTree_VGamma_WWG_%d%s.root",RunYear,whichVFP.c_str())); // Effect in bin 25
			...
			if(sigMET >= lowMET && (highMET < 0|| (highMET > 0 && sigMET < highMET))){
				...
				if( SigBinIndex >= 0){ 
					  ...
					  h_rare_esfUp->Fill( SigBinIndex, weight_scaleup);
					      h_rare_unweighted->Fill(SigBinIndex); 
				}
...
if (sigMTJESup >= lowMt && (highMt < 0 || sigMTJESup < highMt)) {
    if (sigMETJESup >= lowMET && (highMET < 0 || sigMETJESup < highMET)) {
        int bin = Bin.findSignalBin(sigMETJESup, HTJESup, phoEt);
        if (bin >= 0) {
            h_rare_jesUp->Fill(bin, weight);
            if (bin == Bin.findSignalBin(sigMET, HT, phoEt)) {
                h_rare_jesUp_nomstay->Fill(bin, weight); // Fill nomstay only if bin didn’t change
            }
        }
...
if (sigMTJESdo >= lowMt && (highMt < 0 || sigMTJESdo < highMt)) {
    if (sigMETJESdo >= lowMET && (highMET < 0 || sigMETJESdo < highMET)) {
        int bin = Bin.findSignalBin(sigMETJESdo, HTJESdo, phoEt);
        if (bin >= 0) {
            h_rare_jesDown->Fill(bin, weight);
            if (bin == Bin.findSignalBin(sigMET, HT, phoEt)) {
                h_rare_jesDown_nomstay->Fill(bin, weight);
            }
        }
...
if (sigMTJERup >= lowMt && (highMt < 0 || sigMTJERup < highMt)) {
    if (sigMETJERup >= lowMET && (highMET < 0 || sigMETJERup < highMET)) {
        int bin = Bin.findSignalBin(sigMETJERup, HT, phoEt);
        if (bin >= 0) {
            h_rare_jerUp->Fill(bin, weight);
            if (bin == Bin.findSignalBin(sigMET, HT, phoEt)) {
                h_rare_jerUp_nomstay->Fill(bin, weight);
            }
        }
...
if (sigMTJERdo >= lowMt && (highMt < 0 || sigMTJERdo < highMt)) {
    if (sigMETJERdo >= lowMET && (highMET < 0 || sigMETJERdo < highMET)) {
        int bin = Bin.findSignalBin(sigMETJERdo, HT, phoEt);
        if (bin >= 0) {
            h_rare_jerDown->Fill(bin, weight);
            if (bin == Bin.findSignalBin(sigMET, HT, phoEt)) {
                h_rare_jerDown_nomstay->Fill(bin, weight);
            }
        }
   ...
   int sigbin_nom = Bin.findSignalBin(sigMET, HT, phoEt);
    ...
    cout << "NomBin: " << sigbin_nom+19
         << " JESUpBin: " << sigbin_jesup+19
         << " JERUpBin: " << sigbin_jerup+19
         << " JESDownBin: " << sigbin_jesdo+19
         << " JERDownBin: " << sigbin_jerdo+19
    ...
    // JES Up & Down
    float jesUpTotal    = h_rare_jesUp->GetBinContent(contbin);
    float jesUpStay     = h_rare_jesUp_nomstay->GetBinContent(contbin);
    float jesDownTotal  = h_rare_jesDown->GetBinContent(contbin);
    float jesDownStay   = h_rare_jesDown_nomstay->GetBinContent(contbin);

    float jesuperror         = fabs(jesUpTotal    - nominalsig);
    float jesuperror_nomstay = fabs(jesUpStay     - nominalsig);
    float jesdoerror         = fabs(jesDownTotal  - nominalsig);
    float jesdoerror_nomstay = fabs(jesDownStay   - nominalsig);

    float jesTotalSysError = max(jesuperror, jesdoerror);
    float jesStaySysError  = max(jesuperror_nomstay, jesdoerror_nomstay);
    float jesMigEffect     = jesTotalSysError - jesStaySysError;

    // JER Up & Down
    float jerUpTotal    = h_rare_jerUp->GetBinContent(contbin);
    float jerUpStay     = h_rare_jerUp_nomstay->GetBinContent(contbin);
    float jerDownTotal  = h_rare_jerDown->GetBinContent(contbin);
    float jerDownStay   = h_rare_jerDown_nomstay->GetBinContent(contbin);

    float jeruperror         = fabs(jerUpTotal    - nominalsig);
    float jeruperror_nomstay = fabs(jerUpStay     - nominalsig);
    float jerdoerror         = fabs(jerDownTotal  - nominalsig);
    float jerdoerror_nomstay = fabs(jerDownStay   - nominalsig);

    float jerTotalSysError = max(jeruperror, jerdoerror);
    float jerStaySysError  = max(jeruperror_nomstay, jerdoerror_nomstay);
    float jerMigEffect     = jerTotalSysError - jerStaySysError;

    // Set Bin Contents
    h_rare_syserr_jes->SetBinContent(contbin, jesTotalSysError);
    h_rare_syserr_jer->SetBinContent(contbin, jerTotalSysError);

    // Relative uncertainties
    float jesTotalRel = (nominalsig > 0) ? 100.0 * jesTotalSysError / nominalsig : 0.0;
    float jesStayRel  = (nominalsig > 0) ? 100.0 * jesStaySysError  / nominalsig : 0.0;
    float jerTotalRel = (nominalsig > 0) ? 100.0 * jerTotalSysError / nominalsig : 0.0;
    float jerStayRel  = (nominalsig > 0) ? 100.0 * jerStaySysError  / nominalsig : 0.0;


if (jesTotalRel > 100.0 || jerTotalRel > 100.0) {
    float nominal_unweighted = (h_rare_unweighted) ? h_rare_unweighted->GetBinContent(contbin) : -1;

    float jesUp = h_rare_jesUp ? h_rare_jesUp->GetBinContent(contbin) : -1;
    float jesDown = h_rare_jesDown ? h_rare_jesDown->GetBinContent(contbin) : -1;
    float jerUp = h_rare_jerUp ? h_rare_jerUp->GetBinContent(contbin) : -1;
    float jerDown = h_rare_jerDown ? h_rare_jerDown->GetBinContent(contbin) : -1;

    cout << "Bin " << contbin + 18 << " :" << endl;
    cout << "Weighted Yield: " << nominalsig 
         << " jesUp " << jesUp 
         << " jesDown " << jesDown 
         << " jerUp " << jerUp 
         << " jerDown " << jerDown << endl;

    cout << "Unweighted Yield: " << nominal_unweighted << endl;

    cout << "JES Rel: " << std::round(jesTotalRel) << "% (with migration), " 
         << std::round(jesStayRel) << "% (no migration)" << endl;

    cout << "JER Rel: " << std::round(jerTotalRel) << "% (with migration), " 
         << std::round(jerStayRel) << "% (no migration)" << endl;
}
		...
		h_rare_syserr_isr->SetBinContent(contbin, -1);     
	           if (((contbin==7 or contbin==8) and ichannel == 2) or ((contbin==4 or contbin==7)and ichannel == 1))
                cout << contbin << "\t"
                        ...
                        << "h_rare_syserr_jes " << h_rare_syserr_jes->GetBinContent(contbin) << "\t"
                        << "h_rare_syserr_jer " << h_rare_syserr_jer->GetBinContent(contbin) << "\t"
                        << "h_rare_norm " << h_rare_norm->GetBinContent(contbin) << "\t"
                        << "JES unc. (%) " << (100*h_rare_syserr_jes->GetBinContent(contbin))/h_rare_norm->GetBinContent(contbin) << "\t"
                        << "JER unc. (%) " << (100*h_rare_syserr_jer->GetBinContent(contbin))/h_rare_norm->GetBinContent(contbin)
                        << endl;
	}
