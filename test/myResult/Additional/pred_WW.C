...
void pred_WW(){
	...
	if(channelType==1)outputname << "egamma_wwBkg";
	else if(channelType==2)outputname << "mg_wwBkg";
	...
	TH1D *h_ww_jerDown = 0;
	TH1D *h_ww_esfUp = 0;
	TH1D *h_ww_syserr_jes = 0;
	TH1D *h_ww_syserr_jer = 0;
	TH1D *h_ww_syserr_esf = 0;
	TH1D *h_ww_syserr_scale = 0;
	TH1D *h_ww_syserr_eleshape = 0;
	TH1D *h_ww_syserr_jetshape = 0;
	TH1D *h_ww_syserr_qcdshape = 0;
	TH1D *h_ww_syserr_xs = 0;
	TH1D *h_ww_syserr_lumi = 0;
	TH1D *h_ww_syserr_isr = 0;
	if(channelType==1){
		h_ww_norm            = new TH1D("eg_ww_norm","eventcount",NBIN,0,NBIN);
		h_ww_control         = new TH1D("eg_ww_control","eventcount",NBIN,0,NBIN);
		h_ww_jesUp           = new TH1D("eg_ww_jesUp","eventcount",NBIN,0,NBIN);
		h_ww_jesDown         = new TH1D("eg_ww_jesDown","eventcount",NBIN,0,NBIN);
		h_ww_jerUp           = new TH1D("eg_ww_jerUp","eventcount",NBIN,0,NBIN);
		h_ww_jerDown         = new TH1D("eg_ww_jerDown","eventcount",NBIN,0,NBIN);
		h_ww_esfUp           = new TH1D("eg_ww_esfUp","eventcount",NBIN,0,NBIN);
		h_ww_syserr_jes      = new TH1D("eg_ww_syserr_jes","",NBIN,0,NBIN);	
		h_ww_syserr_jer      = new TH1D("eg_ww_syserr_jer","",NBIN,0,NBIN);	
		h_ww_syserr_esf      = new TH1D("eg_ww_syserr_esf","",NBIN,0,NBIN);	
		h_ww_syserr_scale    = new TH1D("eg_ww_syserr_scale","",NBIN,0,NBIN);	
		h_ww_syserr_eleshape = new TH1D("eg_ww_syserr_eleshape","",NBIN,0,NBIN);	
		h_ww_syserr_jetshape = new TH1D("eg_ww_syserr_jetshape","",NBIN,0,NBIN);	
		h_ww_syserr_qcdshape = new TH1D("eg_ww_syserr_qcdshape","",NBIN,0,NBIN);	
		h_ww_syserr_xs       = new TH1D("eg_ww_syserr_xs","",NBIN,0,NBIN);	
		h_ww_syserr_lumi     = new TH1D("eg_ww_syserr_lumi","",NBIN,0,NBIN);
		h_ww_syserr_isr      = new TH1D("eg_ww_syserr_isr","",NBIN,0,NBIN);
	} 
	else if(channelType==2){
		h_ww_norm            = new TH1D("mg_ww_norm","eventcount",NBIN,0,NBIN);
		h_ww_control         = new TH1D("mg_ww_control","eventcount",NBIN,0,NBIN);
		h_ww_jesUp           = new TH1D("mg_ww_jesUp","eventcount",NBIN,0,NBIN);
		h_ww_jesDown         = new TH1D("mg_ww_jesDown","eventcount",NBIN,0,NBIN);
		h_ww_jerUp           = new TH1D("mg_ww_jerUp","eventcount",NBIN,0,NBIN);
		h_ww_jerDown         = new TH1D("mg_ww_jerDown","eventcount",NBIN,0,NBIN);
		h_ww_esfUp           = new TH1D("mg_ww_esfUp","eventcount",NBIN,0,NBIN);
		h_ww_syserr_jes      = new TH1D("mg_ww_syserr_jes","",NBIN,0,NBIN);	
		h_ww_syserr_jer      = new TH1D("mg_ww_syserr_jer","",NBIN,0,NBIN);	
		h_ww_syserr_esf      = new TH1D("mg_ww_syserr_esf","",NBIN,0,NBIN);	
		h_ww_syserr_scale    = new TH1D("mg_ww_syserr_scale","",NBIN,0,NBIN);	
		h_ww_syserr_eleshape = new TH1D("mg_ww_syserr_eleshape","",NBIN,0,NBIN);	
		h_ww_syserr_jetshape = new TH1D("mg_ww_syserr_jetshape","",NBIN,0,NBIN);	
		h_ww_syserr_qcdshape = new TH1D("mg_ww_syserr_qcdshape","",NBIN,0,NBIN);	
		h_ww_syserr_xs       = new TH1D("mg_ww_syserr_xs","",NBIN,0,NBIN);	
		h_ww_syserr_lumi     = new TH1D("mg_ww_syserr_lumi","",NBIN,0,NBIN);
		h_ww_syserr_isr      = new TH1D("mg_ww_syserr_isr","",NBIN,0,NBIN);
	} 
	...
	for(unsigned ievt(0); ievt < mctree->GetEntries(); ievt++){
		...
		if(sigMT >= lowMt && (highMt < 0 || (highMt > 0 && sigMT < highMt)) ){
			...
			if(sigMET >= lowMET && (highMET < 0|| (highMET > 0 && sigMET < highMET))){
				...
				if( SigBinIndex >= 0){ 
					  h_ww_norm->Fill( SigBinIndex, weight);
					  h_ww_control->Fill( SigBinIndex);
					  h_ww_esfUp->Fill( SigBinIndex, weight_scaleup);
				}
		...
		if(sigMTJESup >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJESup < highMt)) ){
			if(sigMETJESup >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJESup < highMET))){
				if(Bin.findSignalBin(sigMETJESup, HTJESup, phoEt)>=0)h_ww_jesUp->Fill( Bin.findSignalBin(sigMETJESup, HTJESup, phoEt),  weight);
		...
		if(sigMTJESdo >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJESdo < highMt)) ){
			if(sigMETJESdo >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJESdo < highMET))){
				if(Bin.findSignalBin(sigMETJESdo, HTJESdo, phoEt)>=0)h_ww_jesDown->Fill( Bin.findSignalBin(sigMETJESdo, HTJESdo, phoEt),  weight);
		...
		if(sigMTJERup >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJERup < highMt)) ){
			if(sigMETJERup >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJERup < highMET))){
				if(Bin.findSignalBin(sigMETJERup, HT, phoEt)>=0)h_ww_jerUp->Fill( Bin.findSignalBin(sigMETJERup, HT, phoEt),       weight);
		...
		if(sigMTJERdo >= lowMt && (highMt < 0 || (highMt > 0 && sigMTJERdo < highMt)) ){
			if(sigMETJERdo >= lowMET && (highMET < 0|| (highMET > 0 && sigMETJERdo < highMET))){
				if(Bin.findSignalBin(sigMETJERdo, HT, phoEt)>=0)h_ww_jerDown->Fill( Bin.findSignalBin(sigMETJERdo, HT, phoEt),       weight); 
	...
	for(int contbin(1); contbin <=NBIN; contbin++){
		binSum= binSum+h_ww_norm->GetBinContent(contbin);
		float nominalsig = h_ww_norm->GetBinContent(contbin); 
	//	std::cout << " bin " << contbin << " ww " << nominalsig << " error " << h_ww_norm->GetBinError(contbin) << " rel error " << h_ww_norm->GetBinError(contbin)/h_ww_norm->GetBinContent(contbin) << " rel error in control region " << sqrt(h_ww_control->GetBinContent(contbin))/h_ww_control->GetBinContent(contbin) << std::endl;
		float jesuperror = fabs(h_ww_jesUp->GetBinContent(contbin)- nominalsig);
		float jesdoerror = fabs(h_ww_jesDown->GetBinContent(contbin)- nominalsig);
		float jeruperror = fabs(h_ww_jerUp->GetBinContent(contbin)- nominalsig);
		float jerdoerror = fabs(h_ww_jerDown->GetBinContent(contbin)- nominalsig);
		float esfuperror = fabs(h_ww_esfUp->GetBinContent(contbin)- nominalsig); 	
	//	std::cout << " jesup " << jesuperror << " jesdo " << jesdoerror << " jerup " << jeruperror << " jerdo " << jerdoerror << std::endl;

		h_ww_syserr_jes->SetBinContent(contbin, max(jesuperror,jesdoerror));
		h_ww_syserr_jer->SetBinContent(contbin, max(jeruperror,jerdoerror));
		h_ww_syserr_esf->SetBinContent(contbin, esfuperror);
		h_ww_syserr_scale->SetBinContent(contbin, -1);
		h_ww_syserr_eleshape->SetBinContent(contbin, -1);
		h_ww_syserr_jetshape->SetBinContent(contbin, -1);
		h_ww_syserr_qcdshape->SetBinContent(contbin, -1);
		h_ww_syserr_xs->SetBinContent(contbin, 0.5*h_ww_norm->GetBinContent(contbin));     
		h_ww_syserr_lumi->SetBinContent(contbin, 0.026*h_ww_norm->GetBinContent(contbin));      
		h_ww_syserr_isr->SetBinContent(contbin, -1);      
	}
