...
void analysis_eg_doubleFake(int RunYear, const char *Era){//main
  ...
  TFile *outputfile = TFile::Open(Form("/eos/uscms/store/user/tmishra/eg_mg_treesData/resTree_egsignal_DoubleEG_%d%s_doubleFake.root",RunYear, Era),"RECREATE");
  outputfile->cd();
...
//*********** histo list **********************//
  ...
  //    if(Nmedpho >= 2)continue;
			...
			for(std::vector<recoPhoton>::iterator itpho = Photon.begin() ; itpho != Photon.end(); ++itpho){
				...
				if((!PixelVeto || !GSFveto)){ // collection for ele->photon fake
...
int main(int argc, char** argv)
{
    ...
    analysis_eg_doubleFake(atoi(argv[1]),argv[2]);
    return 0;
}
