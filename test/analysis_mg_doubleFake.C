...
void analysis_mg_doubleFake(int RunYear, const char *Era){//main
  ...
  TFile *outputfile = TFile::Open(Form("/eos/uscms/store/user/tmishra/eg_mg_treesData/resTree_mgsignal_MuonEG_%d%s_doubleFake.root",RunYear,Era),"RECREATE");
  outputfile->cd();
...
//*********** double fake (fake photon + fake muon) ***********//
  ...
  doubleFaketree->Branch("threeMass", &doubleFakethreeMass);
  ...
  //    if(Nmedpho >= 2)continue;
			...
			for(std::vector<recoPhoton>::iterator itpho = Photon.begin() ; itpho != Photon.end(); ++itpho){
				...
				if(GSFveto && PixelVeto && FSRVeto){
					...
					if(!passSigma || !passChIso){
						if( (itpho->getSigma()< 0.02 && itpho->isEB()) || (itpho->getSigma()< 0.04 && itpho->isEE()) )
							jetPhoCollection.push_back(itpho);}
				}
		...
		if(!hasPho && !hasLep){
			for(unsigned il(0); il < fakeLepCollection.size(); il++){
				std::vector<recoMuon>::iterator fakeMu = fakeLepCollection[il];

				for(unsigned ip(0); ip < proxyPhoCollection.size(); ip++){
					std::vector<recoPhoton>::iterator doubleFakePho = proxyPhoCollection[ip];
					double dRlepphoton = DeltaR(doubleFakePho->getEta(), doubleFakePho->getPhi(), fakeMu->getEta(), fakeMu->getPhi());
					if(dRlepphoton <= 0.8)continue;
					if(!raw.passMETFilter(RunYear, METFilter))continue;

					float doubleFake_deltaPhi = DeltaPhi(fakeMu->getPhi(), METPhi);
					float doubleFake_MT = sqrt(2*MET*fakeMu->getPt()*(1-std::cos(doubleFake_deltaPhi)));
					float doubleFake_ThreeBodyMass = sqrt(2*MET*(doubleFakePho->getP4()+ fakeMu->getP4()).Pt()*(1-std::cos(DeltaR(0, (doubleFakePho->getP4()+fakeMu->getP4()).Phi(), 0, METPhi))));
					...
					doubleFakelepPt = fakeMu->getPt();
					doubleFakelepEta = fakeMu->getEta();
					doubleFakelepPhi = fakeMu->getPhi();
					doubleFakelepMiniIso = fakeMu->getMiniIso();
					doubleFakelepIsStandardProxy = fakeMu->isMedium()? 1:0;

					doubleFakesigMT = doubleFake_MT;
					doubleFakesigMET = MET;
					doubleFakesigMETPhi = METPhi;
					doubleFakedPhiLepMET = doubleFake_deltaPhi;
					doubleFakethreeMass = doubleFake_ThreeBodyMass;
...
int main(int argc, char** argv)
{
    ...
    analysis_mg_doubleFake(atoi(argv[1]),argv[2]);
    return 0;
}
