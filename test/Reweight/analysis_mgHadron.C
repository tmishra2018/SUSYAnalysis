...
void analysis_mgHadron(int RunYear, const char *Era){//main  

  	gSystem->Load("../../lib/libAnaClasses.so");

  	ofstream logfile;
  	logfile.open(Form("/eos/uscms/store/user/tmishra/jetfakepho/logs/plot_hadron_ISR_%d%s.log",RunYear,Era),ios::trunc);
...
{
    analysis_mgHadron(atoi(argv[1]),argv[2]);
    return 0;
}
