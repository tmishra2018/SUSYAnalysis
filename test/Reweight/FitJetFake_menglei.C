...
FitJetFake(float lowercut, float uppercut, int detType){
	...
	datasetname.str("");
	//datasetname << "/eos/uscms/store/user/tmishra/jetfakepho/files/plot_hadron_ISR_2016preVFP.root";
...
//************ Signal Tree **********************//
	...
	mctree->Add("/eos/uscms/store/user/tmishra/jetfakepho/files/plot_hadron_mgGJet_2016preVFP.root");
	float mc_phoEt(0);
	float mc_phoEta(0); 
	float mc_phoPhi(0); 
	float mc_phoSigma(0);
