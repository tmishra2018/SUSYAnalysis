...
void analysis_VGamma_Year17_18(int RunYear, const char *Sample){//main 
        ...
        std::vector<recoPhoton>::iterator mgsignalPho = Photon.begin();
          ...
          if(!itpho->passSignalSelection())continue;
					...
					if(GSFveto && PixelVeto && FSRVeto){
						...
						if(itpho->fireDoubleTrg(27) || itpho->fireDoubleTrg(36)){  // Year = 2017
					 		if(!hasmgPho){
								hasmgPho=true;
					 			mgsignalPho = itpho;
							}
...
//					if((itEle->isEB() && (itEle->getD0() > 0.05 || itEle->getDz() > 0.10)) || (itEle->isEE() && ( itEle->getD0() > 0.10 || itEle->getDz() > 0.20)) )continue;
					if(itEle->fireTrgs(43) || itEle->fireTrgs(44)){   // Year = 2017
        ...
        if(hasmgPho){
					...
					if(hasMu)break;
						if(itMu->fireSingleTrg(0) || itMu->fireSingleTrg(32)){     // Year = 2017
				...
				if(RunYear==2016 && preVFP==1){ 
					Normalization = 0.948822;
					if(ISRJetPt < 50)reweightF = 1.04754;
		                 	else if(ISRJetPt >= 50 && ISRJetPt < 100)reweightF  = 1.16793;
                		 	else if(ISRJetPt >= 100 && ISRJetPt < 150)reweightF = 0.961647;
              			 	else if(ISRJetPt >= 150 && ISRJetPt < 200)reweightF = 0.821928;
                 		 	else if(ISRJetPt >= 200 && ISRJetPt < 250)reweightF = 0.892042;
                 		 	else if(ISRJetPt >= 250 && ISRJetPt < 300)reweightF = 0.865313;
                 		 	else if(ISRJetPt >= 300)reweightF = 0.772129;
				}
				else if(RunYear==2016 && preVFP==0){
					Normalization = 0.871066;
					if(ISRJetPt < 50)reweightF = 1.15039;
			                else if(ISRJetPt >= 50 && ISRJetPt < 100)reweightF  = 1.2914;
                 			else if(ISRJetPt >= 100 && ISRJetPt < 150)reweightF = 0.992371;
                 			else if(ISRJetPt >= 150 && ISRJetPt < 200)reweightF = 0.970282;
                 			else if(ISRJetPt >= 200 && ISRJetPt < 250)reweightF = 0.883577;
                 			else if(ISRJetPt >= 250 && ISRJetPt < 300)reweightF = 0.775856;
                 			else if(ISRJetPt >= 300)reweightF = 0.690008;   
				}
...
int main(int argc, char** argv)
{
    ...
    analysis_VGamma_Year17_18(atoi(argv[1]),argv[2]);
    return 0;
}
