...
void analysis_VGamma(int RunYear, const char *Sample){//main 
  ...
  for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
	...
	double reweightF(1);	
/*			if(preVFP){			
				Normalization = 0.882556;	
				if(ISRJetPt < 50)reweightF = 0.836238;
				else if(ISRJetPt >= 50 && ISRJetPt < 100)reweightF  = 0.986846;
				else if(ISRJetPt >= 100 && ISRJetPt < 150)reweightF = 0.846304;
				else if(ISRJetPt >= 150 && ISRJetPt < 200)reweightF = 0.708233;
				else if(ISRJetPt >= 200 && ISRJetPt < 250)reweightF = 0.749118;
				else if(ISRJetPt >= 250 && ISRJetPt < 300)reweightF = 0.734598;
				else if(ISRJetPt >= 300)reweightF =  0.707612; }
			else{				
				Normalization = 1.01492;
				if(ISRJetPt < 50)reweightF = 1.00611;
				else if(ISRJetPt >= 50 && ISRJetPt < 100)reweightF  = 1.11986;
				else if(ISRJetPt >= 100 && ISRJetPt < 150)reweightF = 0.924251;
				else if(ISRJetPt >= 150 && ISRJetPt < 200)reweightF = 0.904514;
				else if(ISRJetPt >= 200 && ISRJetPt < 250)reweightF = 0.936735;
				else if(ISRJetPt >= 250 && ISRJetPt < 300)reweightF = 0.773806;
				else if(ISRJetPt >= 300)reweightF =  0.551791; }
...
//**********************  mg filter **************************************//         
       if(hasmgPho && hasMu &&  ((RunYear == 2016 && (((raw.HLTEleMuX >> 8)&1)!=0 || ((raw.HLTEleMuX >> 51)&1)!=0)) || ((RunYear == 2017 || RunYear == 2018) && (((raw.HLTEleMuX >> 8)&1)!=0 || ((raw.HLTEleMuX >> 57)&1)!=0))) && !hasDoubleEG){
