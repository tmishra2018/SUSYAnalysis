...
void analysis_VGamma(int RunYear, const char *Sample){//main 
  ...
  ofstream logfile;
  logfile.open(Form("/eos/uscms/store/user/tmishra/VGamma/resTree_VGamma_%s_%dpreVFP.log",Sample,RunYear)); 
  ...
  if(RunYear==2016){
		 ...
		 Pass_egPho = "(itpho->fireDoubleTrg(5) || itpho->fireDoubleTrg(6))";
  ...
  TFile* outputfile = new TFile(Form("/eos/uscms/store/user/tmishra/VGamma/resTree_VGamma_%s_%dpreVFP_mycode.root",Sample,RunYear),"RECREATE");
  outputfile->cd();
...
//					if((itEle->isEB() && (itEle->getD0() > 0.05 || itEle->getDz() > 0.10)) || (itEle->isEE() && ( itEle->getD0() > 0.10 || itEle->getDz() > 0.20)) )continue;
        ...
        if(hasmgPho){
				...
				// ISR weight from table 14 analysis note
                        if(preVFP){
                                if(ISRJetPt < 50)reweightF = 1.16982;
                                else if(ISRJetPt >= 50 && ISRJetPt < 100)reweightF  = 1.2639;
                                else if(ISRJetPt >= 100 && ISRJetPt < 150)reweightF = 0.998353;
                                else if(ISRJetPt >= 150 && ISRJetPt < 200)reweightF = 0.833455;
                                else if(ISRJetPt >= 200 && ISRJetPt < 250)reweightF = 0.854334;
                                else if(ISRJetPt >= 250 && ISRJetPt < 300)reweightF = 0.825496;
                                else if(ISRJetPt >= 300)reweightF =  0.819437; }
                        else{
                                if(ISRJetPt < 50)reweightF = 1.19389;
                                else if(ISRJetPt >= 50 && ISRJetPt < 100)reweightF  = 1.3276;
                                else if(ISRJetPt >= 100 && ISRJetPt < 150)reweightF = 1.03011;
                                else if(ISRJetPt >= 150 && ISRJetPt < 200)reweightF = 0.999649;
                                else if(ISRJetPt >= 200 && ISRJetPt < 250)reweightF = 1.02922;
                                else if(ISRJetPt >= 250 && ISRJetPt < 300)reweightF = 0.837899;
                                else if(ISRJetPt >= 300)reweightF =  0.595739; }
...
logfile <<"Entries in egtree "<<egtree->GetEntries()<<"  "<<100.*egtree->GetEntries()/es->GetEntries()<<endl;
logfile <<"Entries in mgtree "<<mgtree->GetEntries()<<"  "<<100.*mgtree->GetEntries()/es->GetEntries()<<endl;
outputfile->Write();
outputfile->Close();
logfile.close();
