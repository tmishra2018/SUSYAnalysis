...
void plotGauss(int RunYear, int preVFP, int isQCD, int ichannel){
	...
	for(unsigned i(0);  i < 1; i++){
		...
		if(isQCD==1){
			lowRange = fakescale-0.3*fakescale;
			highRange = fakescale+0.3*fakescale;}
			//lowRange = fakescale-0.3;
			//highRange = fakescale+0.3;}
		else{
			lowRange = vgammascale-0.3*vgammascale;
			highRange = vgammascale+0.3*vgammascale;}
			//lowRange = vgammascale-0.7;
			//highRange = vgammascale+0.7;}
	}
	cout<<"lowRange = "<<lowRange <<" highRange :"<<highRange<<endl;

	TH1F *p_frac_0;
	p_frac_0 = new TH1F("p_frac_0","",100,lowRange,highRange);
	//if(isQCD==true)	p_frac_0 = new TH1F("p_frac_0","",100,0.3,0.8);
	//if(isQCD==1)	p_frac_0 = new TH1F("p_frac_0","",100,0.0,0.8);
	//if(isQCD==1)	p_frac_0 = new TH1F("p_frac_0","",100,0.0,1.5);
	//else		p_frac_0 = new TH1F("p_frac_0","",100,1.0,2.7);
	...
	cout<<h_mean <<"\t"<<h_std<<endl;
	p_frac_0->Fit("gaus","0","", h_mean-0.1, h_mean+0.1);
