...
bool doTT = false;
void plot_bkg_doubleFake(int channel,int RunYear,bool preVFP){//main  
	SetRunConfig();
	...
	std::ostringstream dfname;   dfname.str("");
	...
	if(channel == 1){
		if(plottype == 1){
			...
			dfname <<  "/uscms_data/d3/tmishra/Background/bkgTree_egamma_doubleFake_met"<< lowMET<<"_"<<highMET<<"_pt0_1000_" << RunYear<<whichVFP <<".root";
		}
		else if(plottype == 2){
			...
			dfname << "/uscms_data/d3/tmishra/Background/validTree_egamma_doubleFake_" << RunYear<<whichVFP <<".root";
		}
	}
	else if(channel == 2){
		if(plottype == 1){
			...
			dfname << "/uscms_data/d3/tmishra/Background/bkgTree_mg_doubleFake_met"<< lowMET<<"_"<<highMET<<"_pt0_1000_" << RunYear<<whichVFP <<".root";
		}
		else if(plottype == 2){
			...
			dfname << "/uscms_data/d3/tmishra/Background/validTree_mg_doubleFake_" << RunYear<<whichVFP <<".root";
		}
	}
	...
	TFile *file_df  = TFile::Open(dfname.str().c_str());
	...
	TH1F *p_dfPhoEt = (TH1F*)file_df->Get("p_PhoEt");
	TH1F *p_dfLepPt = (TH1F*)file_df->Get("p_LepPt");
	TH1F *p_dfMET   = (TH1F*)file_df->Get("p_MET");
	TH1F *p_dfMt    = (TH1F*)file_df->Get("p_Mt");
	TH1F *p_dfHT    = (TH1F*)file_df->Get("p_HT");
	TH1F *p_dfPU    = (TH1F*)file_df->Get("p_PU");
	...
	p_dfMt->SetFillStyle(1001);
	p_dfMt->SetLineColor(kGray+2);
	p_dfMt->SetFillColor(kGray+2);
	...
	float events_eleMt, events_jetMt, events_qcdMt, events_VGMt, events_rareMt, events_allMt, events_dfMt;
	...
	p_qcdMt->Add(p_dfMt, -1.0); // subtract double-fake overlap
	...
	leg_mt->AddEntry(p_dfMt,Form("double fake [%.1f] subtraction",events_dfMt));
	...
	p_dfPhoEt->SetFillStyle(1001);
	p_dfPhoEt->SetLineColor(kGray+2);
	p_dfPhoEt->SetFillColor(kGray+2);
	...
	float events_elePhoEt, events_jetPhoEt, events_qcdPhoEt, events_VGPhoEt, events_rarePhoEt, events_allPhoEt, events_dfPhoEt;
	...
	events_dfPhoEt = p_dfPhoEt->Integral();
	...
	p_qcdPhoEt->Add(p_dfPhoEt, -1.0); // subtract double-fake overlap
	...
	p_dfMET->SetFillStyle(1001);
	p_dfMET->SetLineColor(kGray+2);
	p_dfMET->SetFillColor(kGray+2);
	...
	cout<<"MET:\t" <<events_eleMET <<"\t"<<events_jetMET<<"\t"<<events_qcdMET<<"\t"<<events_VGMET<<"\t"<<events_rareMET<<"\t"<<events_allMET<<endl;		
		...
		p_qcdMET->Add(p_dfMET, -1.0); // subtract double-fake overlap
	...
	// ******** HT ************************//
		...
		p_dfHT->SetFillStyle(1001);
		p_dfHT->SetLineColor(kGray+2);
		p_dfHT->SetFillColor(kGray+2);
		...
		p_qcdHT->Add(p_dfHT, -1.0); // subtract double-fake overlap
		...
		p_eleMET_HT->Add(p_rareMET_HT); // ele 2nd
		p_jetMET_HT->Add(p_eleMET_HT);  // jet 3rd
		p_qcdMET_HT->Add(p_jetMET_HT);  // qcd 4th
		p_VGMET_HT->Add(p_qcdMET_HT);   // VG  5th
		p_VGMET_HT->Sumw2();
		for(int ibin(1); ibin < p_VGMET_HT->GetSize(); ibin++){
			float prederror = p_VGMET_HT->GetBinError(ibin); // Quadrature sum of individual errors, checked
			error_MET_HT->SetPoint(ibin-1,p_VGMET_HT->GetBinCenter(ibin), p_VGMET_HT->GetBinContent(ibin));
			error_MET_HT->SetPointError(ibin-1,(p_VGMET_HT->GetBinLowEdge(ibin+1)-p_VGMET_HT->GetBinLowEdge(ibin))/2,prederror);
			ratioerror_MET_HT->SetPoint(ibin-1,p_VGMET_HT->GetBinCenter(ibin), 1);
			ratioerror_MET_HT->SetPointError(ibin-1,(p_VGMET_HT->GetBinLowEdge(ibin+1)-p_VGMET_HT->GetBinLowEdge(ibin))/2, prederror/p_VGMET_HT->GetBinContent(ibin));
		}
		p_VGMET_HT->Draw("hist same");
		p_qcdMET_HT->Draw("hist same");
		p_jetMET_HT->Draw("hist same");
		p_eleMET_HT->Draw("hist same");
		p_rareMET_HT->Draw("hist same");
		...
		ratio_metht->GetYaxis()->SetRangeUser(0.,2.0);
		...
		p_eleHT_MET->Add(p_rareHT_MET); // ele 2nd
		p_jetHT_MET->Add(p_eleHT_MET);  // jet 3rd
		p_qcdHT_MET->Add(p_jetHT_MET);  // qcd 4th
		p_VGHT_MET->Add(p_qcdHT_MET);   // VG  5th
		p_VGHT_MET->Sumw2();
		for(int ibin(1); ibin < p_VGHT_MET->GetSize(); ibin++){
			float prederror = p_VGHT_MET->GetBinError(ibin); // Quadrature sum of individual errors, checked
			error_HT_MET->SetPoint(ibin-1,p_VGHT_MET->GetBinCenter(ibin), p_VGHT_MET->GetBinContent(ibin));
			error_HT_MET->SetPointError(ibin-1,(p_VGHT_MET->GetBinLowEdge(ibin+1)-p_VGHT_MET->GetBinLowEdge(ibin))/2,prederror);
			ratioerror_HT_MET->SetPoint(ibin-1,p_VGHT_MET->GetBinCenter(ibin), 1);
			ratioerror_HT_MET->SetPointError(ibin-1,(p_VGHT_MET->GetBinLowEdge(ibin+1)-p_VGHT_MET->GetBinLowEdge(ibin))/2, prederror/p_VGHT_MET->GetBinContent(ibin));
		}
		p_VGHT_MET->Draw("hist same");
		p_qcdHT_MET->Draw("hist same");
		p_jetHT_MET->Draw("hist same");
		p_eleHT_MET->Draw("hist same");
		p_rareHT_MET->Draw("hist same");
		...
		ratio_htmet->GetYaxis()->SetRangeUser(0.,2.0);
	...
	//        p_allLepPt->GetXaxis()->SetRangeUser(35,800);
		...
		p_dfLepPt->SetFillStyle(1001);
		p_dfLepPt->SetLineColor(kGray+2);
		p_dfLepPt->SetFillColor(kGray+2);
		...
		p_qcdLepPt->Add(p_dfLepPt, -1.0); // subtract double-fake overlap
