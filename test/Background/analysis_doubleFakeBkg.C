...
void analysis_doubleFakeBkg(){
	...
	double factorQCD(1.0);
	if(channelType == 1)factorQCD = factor_egQCD;
	else if(channelType == 2)factorQCD = factor_mgQCD;
	if(anatype == 0)factorQCD = 1.0;
	...
	TH1D *p_scale(0);
	if(scaleFile != 0)p_scale = (TH1D*)scaleFile->Get("transfer_factor");
	...
	elefake_file.close();
	TF3 h_elefakerate("h_elefakerate", fakerate_func,10,1000,0,100,0,1.5,7);
	h_elefakerate.SetParameters(scalefactor, ptslope, ptconstant, ptindex, ptcoeff, vtxconst, vtxslope);
	...
	for(unsigned ievt(0); ievt < doubleFaketree->GetEntries(); ievt++){
		...
		// lepton proxy requirement (same logic as analysis_qcdBkg.C)
		bool isProxy(false);
		...
		double w_lep(0);
		if(channelType == 1 && p_scale != 0)w_lep = factorQCD*p_scale->GetBinContent(p_scale->FindBin(lepPt));
		else w_lep = factorQCD;

		double w_pho(0);
		if(phoType == 0){
			w_pho = h_elefakerate(phoEt, nVertex, fabs(phoEta));
