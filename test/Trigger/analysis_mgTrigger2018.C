...
void analysis_mgTrigger2018(int RunYear, const char *Era){//main  
	...
	for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries
		...
		if(ievt%100000==0) std::cout << " -- Processing event " << ievt << " finish " << 1.0*ievt/nEvts*100 << "%" << std::endl;
			...
			if(((raw.HLTEleMuX >> 19) &1)==0 && ((raw.HLTEleMuX >> 20) &1)==0 && ((raw.HLTEleMuX >> 21) &1)==0)continue;
			Z_nVtx= raw.nVtx;
			Z_rho = raw.rho;
			Z_run = raw.run; 
			...
			for(std::vector<recoMuon>::iterator itLeadMu = Muon.begin(); itLeadMu!= Muon.end(); ++itLeadMu){
				if(!itLeadMu->fireSingleTrg(0) && !itLeadMu->fireSingleTrg(1) && !itLeadMu->fireSingleTrg(17) && !itLeadMu->fireSingleTrg(30) && !itLeadMu->fireSingleTrg(31))continue;
            ...
            if(probePho->fireDoubleTrg(29))Z_phofireHLT=1;
						else Z_phofireHLT=0;
			...
			if(hasMu && probePhoVec.size() > 0){
				//for(int ipho(0); ipho < probePhoVec.size(); ipho++){
					...
					if(dR > 0.8){ 
						...
						if(itIsoPho->fireDoubleTrg(29))mg_phofireHLT=1;
