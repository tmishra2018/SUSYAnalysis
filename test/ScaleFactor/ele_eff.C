...
void ele_eff(){
	...
	//*********** histo list **********************//
	TH2D *p_eff = new TH2D("p_eff","Efficiency;Electron p_{T} (GeV);#eta",5,0,5,5,0,5);
	double lep_eff[5][5];
	double mc_count[5][5];
...
//
  ...
  mctree->SetBranchAddress("mcMomPID",  &mcMomPID);
	...
	for(unsigned ievt(0); ievt < mctree->GetEntries(); ievt++){
		...
		if(fabs(mcEta) < 0.8)
			etabin = 0;
		else if(fabs(mcEta) > 0.8 && fabs(mcEta) < 1.442)
			etabin = 1;
		else if(fabs(mcEta) > 1.442 && fabs(mcEta) < 1.556)
			etabin = 2;
		else if(fabs(mcEta) > 1.556 && fabs(mcEta) < 2.0)
			etabin = 3;
		else if(fabs(mcEta) > 2.0 && fabs(mcEta) <= 2.5)
			etabin = 4;

		if(fabs(mcEta) > 2.5)continue;
	...
	double ptlist[] = {25,35,45,70,120};
	double etalist[] = {0.4, 1.2, 1.450, 1.8, 2.1};
	for(unsigned i(0); i<5; i++){
		for(unsigned j(0); j<5; j++){
			double pt = ptlist[i];
  ...
  can->SetRightMargin(0.1);
	...
	p_eff->GetYaxis()->SetBinLabel(1,"0-0.8");
	p_eff->GetYaxis()->SetBinLabel(2,"0.8-1.442");
	p_eff->GetYaxis()->SetBinLabel(3,"1.442-1.556");
	p_eff->GetYaxis()->SetBinLabel(4,"1.556-2.0");
	p_eff->GetYaxis()->SetBinLabel(5,"2.0-2.5");
	
	gStyle->SetPaintTextFormat("4.3f");
	p_eff->GetZaxis()->SetRangeUser(0,1);
	p_eff->Draw("colz text E");
	can->SaveAs("/eos/uscms/store/user/tmishra/Trigger/eff_el_17012.pdf");

	TFile *outputfile = new TFile("/eos/uscms/store/user/tmishra/Trigger/eff_el_17012.root","RECREATE");
	outputfile->cd();
	p_eff->Write();
	outputfile->Close();	
}
