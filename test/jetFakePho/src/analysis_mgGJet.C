...
void analysis_mgGJet(int RunYear){//main  
  ...
  ofstream logfile;
  logfile.open(Form("/eos/uscms/store/user/tmishra/jetfakepho/logs/plot_hadron_mgGJets_%d%s.log",RunYear,whichVFP.c_str()),ios::trunc);
  ...
  TFile *outputfile;
  outputfile = TFile::Open(Form("/eos/uscms/store/user/tmishra/jetfakepho/files/plot_hadron_mgGJet_%d%s.root",RunYear,whichVFP.c_str()),"RECREATE");
  outputfile->cd();
