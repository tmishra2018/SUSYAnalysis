...
#define NBIN 17
#define REBINSIZE 1
Double_t tmpjetfake_func_den(Double_t *x, Double_t *par)
{
...
void fitJetFunc(int ichannel,int  RunYear,bool ISpreVFP){
	...
	TF1 *fitfunc_den= new TF1("fitfunc_den", tmpjetfake_func_den, 35, 300, 4);
	...
	latex_den->DrawLatex(70,0.2,hname.str().c_str());
	...
	for(int i(0); i<NTOY; i++){
		...
		gen_den[i] = new TF1(modelnameden.str().c_str(), tmpjetfake_func_den, 35, 300, 4);
