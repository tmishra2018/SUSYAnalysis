...
void fitJetFunc_Original(int ichannel,int  RunYear,bool ISpreVFP){
	int detType = 1;
	...
	for(unsigned ibin(76); ibin <= 94; ibin++){
		...
		cout<<ibin <<"  "<< new_fakes_value << "  "<<new_fakes_error<<endl;
	}
	{ // overflow bin
...
if(channel == 1)fitfunc_num->SetParameters(14300, 300, -0.03, -0.07);
