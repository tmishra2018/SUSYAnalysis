...
void fitJetFunc_correct_paravalue(int ichannel,int  RunYear,bool ISpreVFP){
	int detType = 1;
...
// Numerator toy value storage
float toyptvalue[565][NTOY];
float lowtoyptvalue[565];
float hightoyptvalue[565];

// Denominator toy value storage - ADD THESE
float dentoyptvalue[565][NTOY];
float lowdentoyptvalue[565];
float highdentoyptvalue[565];

// Initialize all arrays
for(unsigned ii(0); ii < 565; ii++){
    lowtoyptvalue[ii] = 100000;
    hightoyptvalue[ii] = 0;
    lowdentoyptvalue[ii] = 100000;
    highdentoyptvalue[ii] = 0;
}
// DENOMINATOR TOY LOOP - Updated
for(int i(0); i<NTOY; i++){
    ...
    // Store denominator toy values - Changed from ibin(1) to ibin(0) to match numerator indexing
    for(unsigned ibin(0); ibin < 565; ibin++){
        double estimated = gen_den[i]->Eval(35+ibin);  // Changed to match numerator: 35+ibin
        
        if (std::isnan(estimated) || std::isinf(estimated)) {
            std::cout << "[WARNING] Denominator fit returned non-finite value at bin " << ibin
                << " for toy " << i << std::endl;
            continue;
        }
        
        dentoyptvalue[ibin][i] = estimated;  // STORE DENOMINATOR TOY VALUE
        if(lowdentoyptvalue[ibin] > estimated) lowdentoyptvalue[ibin] = estimated;
        if(highdentoyptvalue[ibin] < estimated) highdentoyptvalue[ibin] = estimated;
    }
}
...
for(int i(0); i<NTOY; i++){
    ...
    if(data1 < 1e8 && data2 < 1e8 && data3 < 1e8 && data4 < 1e8 && data5 < 1e8 && data6 < 1e8){
        ...
        if(exception) continue;  // Moved outside inner loop
    }
...
// Calculate and write NUMERATOR standard deviation
for(unsigned ii(0); ii < 265; ii++){
    ...
    for(int itoy = 0; itoy < NTOY; itoy++){
        if(std::isfinite(toyptvalue[ii][itoy])){
            mean_num += toyptvalue[ii][itoy];
            valid_toys++;
        }
    }
    if(valid_toys > 0) mean_num /= valid_toys;

    // Calculate variance
    double var_num = 0;
    for(int itoy = 0; itoy < NTOY; itoy++){
        if(std::isfinite(toyptvalue[ii][itoy])){
            double diff = toyptvalue[ii][itoy] - mean_num;
            var_num += diff * diff;
        }
    }

    // Calculate standard deviation
    double sigma_num = (valid_toys > 1) ? sqrt(var_num / (valid_toys - 1)) : 0;

    myfile << "numerror " << ii << " " << sigma_num << std::endl;
}

// Calculate and write DENOMINATOR standard deviation
for(unsigned ii(0); ii < 265; ii++){
    double mean_den = 0;
    int valid_toys = 0;
    ...
    // Calculate mean
    for(int itoy = 0; itoy < NTOY; itoy++){
        if(std::isfinite(dentoyptvalue[ii][itoy])){
            mean_den += dentoyptvalue[ii][itoy];
            valid_toys++;
        }
    }
    if(valid_toys > 0) mean_den /= valid_toys;

    // Calculate variance
    double var_den = 0;
    for(int itoy = 0; itoy < NTOY; itoy++){
        if(std::isfinite(dentoyptvalue[ii][itoy])){
            double diff = dentoyptvalue[ii][itoy] - mean_den;
            var_den += diff * diff;
        }
    }

    // Calculate standard deviation
    double sigma_den = (valid_toys > 1) ? sqrt(var_den / (valid_toys - 1)) : 0;

    myfile << "denerror " << ii << " " << sigma_den << std::endl;
}
