...
int FitJetFake_HTbinned(int  RunYear, bool ISpreVFP, int  eventType, float lowercut, float uppercut, int detType, float loweta, float higheta){
	...
	myfilename << "/eos/uscms/store/user/tmishra/jetfakepho/txt"<<RunYear<< whichVFP<<"/JetFakeRate-" << outputType << outputDet << "_HTbinned.txt";
	Hist1Dname << "/eos/uscms/store/user/tmishra/jetfakepho/Plots"<<RunYear<< whichVFP<<"/"<<outputT<<"/frac-" << lowername << "-" << uppername << "-" << outputType << outputDet << "_HTbinned.png";
	Hist2Dname << "/eos/uscms/store/user/tmishra/jetfakepho/Plots"<<RunYear<< whichVFP<<"/"<<outputT<<"/can2D-" << lowername << "-" << uppername << "-" << outputType << outputDet << "_HTbinned.png";
	...
	switch(eventType){
		...
		case 2: datatree->Add(Form("/eos/uscms/store/user/tmishra/eg_mg_treesData/NEW/resTree_mgsignal_MuonEG_%d%s.root",RunYear,whichVFP.c_str())); break;
	...
	elefake_config.str("");
	elefake_config << "/eos/uscms/store/user/tmishra/elefakepho/DATAResult"<<RunYear<<whichVFP<<"/EleFakeRate-Data-ByPt-" << outputDet <<".txt";
...
//cut based photon ID 94X-V2 ID for Run2
	...
	TFile *outputfile;
	outputfile = TFile::Open(Form("/uscms_data/d3/tmishra/jetfakepho/JetFakeRate-%s%d%s_HTbinned.root",outputName.c_str(),RunYear,whichVFP.c_str()),"RECREATE");
	outputfile->cd();
	hname.str("");
...
//************ data Tree **********************//
	...
	datatree->SetBranchAddress("HT",        &HT);
	...
	for(unsigned ievt(0); ievt < datatree->GetEntries(); ievt++){
		...
		for(unsigned iele(0); iele < eleproxyEt->size(); iele++){
			...
			double weight_pt = fakerate_pt((*eleproxyEt)[iele], ptslope, ptconstant, ptindex);
			double weight_eta = getBinnedWeight(fabs((*eleproxyEta)[iele]), etaBins, etaWeights);
			double weight_ht = getBinnedWeight(HT, htBins, htWeights);
			double w_ele = -1.0*scalefactor*weight_pt*weight_eta*weight_ht;
