...
#define NBIN 9
#define REBINSIZE 1
Double_t tmpjetfake_func_2Expo(Double_t *x, Double_t *par)
{
...
Double_t tmpjetfake_func(Double_t *x, Double_t *par)
{
	...
	double jetfakes_lowedge = c1*exp(lamda1*pt_low)/lamda1 + c2*exp(lamda2*pt_low)/lamda2 + c3*exp(lamda3*pt_low)/lamda3;
	double jetfakes_highedge =  c1*exp(lamda1*pt_high)/lamda1 + c2*exp(lamda2*pt_high)/lamda2 + c3*exp(lamda3*pt_high)/lamda3;
	return (jetfakes_highedge - jetfakes_lowedge);
}
Double_t tmp_func_expo(Double_t *x, Double_t *par){
	return par[0]*exp(par[1]*x[0])/par[1];	 }

Double_t tmp_func_2expo(Double_t *x, Double_t *par){
	return par[0]*exp(par[2]*x[0])/par[2] + par[1]*exp(par[3]*x[0])/par[3];	 }

Double_t tmp_func_3expo(Double_t *x, Double_t *par){
	return par[0]*exp(par[3]*x[0])/par[3] + par[1]*exp(par[4]*x[0])/par[4] + par[2]*exp(par[5]*x[0])/par[5];	 }

// root -l -q "fitJetFunc.C(1,2016,1)"
void fitJetFunc_ratio(int ichannel,int  RunYear,bool ISpreVFP){
	int detType = 1;
	...
	for(unsigned ibin(1); ibin <= 75; ibin++){
		...
		//cout<< p_fakesPhoEt->GetBinCenter(ibin) <<"  "<< new_fakes_value << endl;
	...
	for(unsigned ibin(76); ibin <= 94; ibin++){
		...
		//cout<< p_fakesPhoEt->GetBinCenter(80+(ibin-76)*10) <<"  "<< new_fakes_value <<endl;
	}
	{ // overflow bin
	...
	TGraphAsymmErrors* new_fakeRatePhoEt = new TGraphAsymmErrors();
	for (int i = 0; i < new_fakesPhoEt->GetN(); i++) {
  		double x1, x2, y1, y2;

  		new_fakesPhoEt->GetPoint(i, x1, y1);
  		new_controlPhoEt->GetPoint(i, x2, y2);
  		double ratio = (y2 != 0) ? y1 / y2 : 0; // handle division by zero

  		double errUp1 = new_fakesPhoEt->GetErrorYhigh(i);
  		double errDown1 = new_fakesPhoEt->GetErrorYlow(i);
  		double errUp2 = new_controlPhoEt->GetErrorYhigh(i);
  		double errDown2 = new_controlPhoEt->GetErrorYlow(i);

  		double errUp = (y2 != 0) ? ratio * sqrt(pow(errUp1 / y1, 2) + pow(errUp2 / y2, 2)) : 0;
  		double errDown = (y2 != 0) ? ratio * sqrt(pow(errDown1 / y1, 2) + pow(errDown2 / y2, 2)) : 0;
		cout<<x1<<"\t"<<ratio<<endl;
  		new_fakeRatePhoEt->SetPoint(i, x1, ratio);
  		new_fakeRatePhoEt->SetPointError(i, 0, 0, errDown, errUp);
	}



	//new_fakeRatePhoEt->Divide(new_fakesPhoEt, new_controlPhoEt);
	
	new_fakeRatePhoEt->GetXaxis()->SetTitle("p_{T} (GeV)");
	new_fakeRatePhoEt->GetXaxis()->SetTitleOffset(1);
	new_fakeRatePhoEt->GetXaxis()->SetTitleSize(20);
	new_fakeRatePhoEt->GetXaxis()->SetRangeUser(35,300);
...
//	new_fakesPhoEt->Draw("EP same");
	new_fakeRatePhoEt->Draw("EP same");
...
//	leg->AddEntry(new_fakesPhoEt,"fake photons");
	leg->AddEntry(new_fakeRatePhoEt,"fake rate");
	leg->Draw("same");
	...
	TF1 *double_expo1 = new TF1("double_expo1","expo",35,80);
	TF1 *double_expo2 = new TF1("double_expo2","expo",90,200);
	TF1 *total = new TF1("total","expo(0)+expo(3)",35,200);
	
	//TF1 *double_expo1 = new TF1("double_expo1",tmp_func_2expo,35,80,4);
	//TF1 *double_expo2 = new TF1("double_expo2",tmp_func_2expo,90,200,4);
	//TF1 *total = new TF1("total","tmp_func_2expo(0)+gaus(4)",35,200);
	Double_t *par;
	double_expo1->GetParameters(&par[0]);
   	double_expo2->GetParameters(&par[3]);
	total->SetParameters(par);

	//new_fakeRatePhoEt->Fit(double_expo1,"R");
	//new_fakeRatePhoEt->Fit(double_expo2,"R+");
	//new_fakeRatePhoEt->Fit(total,"R+");

//	new_fakeRatePhoEt->GetXaxis()->SetRangeUser(35, 80);
//	new_fakeRatePhoEt->Fit("expo");
//	TF1 *inifitden = new_fakeRatePhoEt->GetFunction("expo");
//	inifitden->Draw("same");
...
//	cout<<"iniLambda_den2  : "<<iniLambda_den2<<endl; 
	...
	TF1 *fitfunc_den= new TF1("fitfunc_den", tmpjetfake_func_2Expo, 35, 300, 4);
...
//	TF1 *func_expo 	= new TF1("func_expo", tmp_func_expo, 90, 200, 2);
//	func_expo->SetParameters(iniCoeff_den, iniCoeff_den/10);

//	new_fakeRatePhoEt->GetXaxis()->SetRangeUser(90, 300);
//	TF1 *func_2expo = new TF1("func_2expo", tmp_func_2expo, 90, 300, 4);
//	func_2expo->SetParameters(iniCoeff_den, iniCoeff_den/10, iniLambda_den1, iniLambda_den1);
	
	//TF1 *func_3expo = new TF1("func_3expo", tmp_func_3expo, 35, 300, 6);
	//func_3expo->SetParameters(iniCoeff_den, iniCoeff_den/10,  iniCoeff_den/10, iniLambda_den1, iniLambda_den1, iniLambda_den1);

	//new_fakeRatePhoEt->GetXaxis()->SetRangeUser(35, 80);
	//new_fakeRatePhoEt->Fit("func_expo","S same");
//	new_fakeRatePhoEt->Fit("func_2expo","R+ S same");
//	TF1 *fitden = new_fakeRatePhoEt->GetFunction("func_2expo");
	//func_expo->Draw("same");
	//new_fakeRatePhoEt->Fit("func_3expo","S  same");
	
	//new_fakeRatePhoEt->Fit("fitfunc_den","S");
	TF1 *fitden = new_fakeRatePhoEt->GetFunction("fitfunc_den");
	...
	rden->Print("V");
	double_expo1->Draw("same");
	double_expo2->Draw("same");
	//total->Draw("same");
	//func_expo->Draw("same");
	//func_2expo->Draw("same");
	//func_3expo->Draw("same");
	TVectorD muden(4) ;
	...
	TLatex* latex_den = new TLatex();
	latex_den->SetTextColor(kBlack);
	latex_den->SetTextSize(0.03);
	hname.str("");
	float chindf_den = fitden->GetChisquare()/fitden->GetNDF();
	cout << "chiSquare:  " << fitden->GetChisquare() << std::endl;
	cout << "ndf:  " << fitden->GetNDF() << std::endl;
	hname << "#chi^{2}/ndof = " << fitden->GetChisquare()/fitden->GetNDF();
//	latex_den->DrawLatex(70,0.01,hname.str().c_str());
	...
	for(int i(0); i<NTOY; i++){
		...
		gen_den[i] = new TF1(modelnameden.str().c_str(), tmpjetfake_func_2Expo, 35, 300, 4);
...
TF1 *fitfunc_num= new TF1("fitfunc_num", tmpjetfake_func, 35, 300, 6);
...
if(channel == 1)fitfunc_num->SetParameters(14300, 300, 500, -0.070, -0.04,-0.02);
//else if(channel == 2)fitfunc_num->SetParameters(iniCoeff_num, iniCoeff_num/10, iniLambda_num1, iniLambda_num2);
else if(channel == 2) fitfunc_num->SetParameters(23900, 300, 100, -0.082, -0.01,-0.04);
...
//TFitResultPtr r = new_fakeRatePhoEt->Fit("fitfunc_num","R S");
//TF1 *fit = new_fakeRatePhoEt->GetFunction("fitfunc_num");

//new_fakeRatePhoEt->Fit("fitfunc_num","S");
...
cout << "num_coeff3 " << fit->GetParameter(2) << std::endl;
cout << "num_lambd1 " << fit->GetParameter(3) << std::endl;
cout << "num_lambd2 " << fit->GetParameter(4) << std::endl;
cout << "num_lambd3 " << fit->GetParameter(5) << std::endl;

cout << "chiSquare/ndf:  " << fit->GetChisquare()/fit->GetNDF() << std::endl;
cout << "chiSquare:  " << fit->GetChisquare() << std::endl;

//latex->DrawLatex(1,0.000002*new_dummy->GetMaximum(),hname.str().c_str());
...
/*
	...
	latex->DrawLatex(70,0.4,hname.str().c_str());
	...
	RooRealVar central_coeff3_num("central_coeff3_num","central_coeff3_num",mu(2)-r->ParError(2), mu(2)+r->ParError(2));
	RooRealVar central_lambda1_num("central_lambda1_num","central_lambda1_num",mu(3)-r->ParError(3), mu(3)+r->ParError(3));
	RooRealVar central_lambda2_num("central_lambda2_num","central_lambda2_num",mu(4)-r->ParError(4), mu(4)+r->ParError(4));
	RooRealVar central_lambda3_num("central_lambda3_num","central_lambda3_num",mu(5)-r->ParError(5), mu(5)+r->ParError(5));
	
	RooMultiVarGaussian mvg("mvg","mvg",RooArgList(central_coeff1_num,central_coeff2_num,central_coeff3_num,central_lambda1_num,central_lambda2_num,central_lambda3_num),mu,cov);
	RooDataSet* toymcdata = mvg.generate(RooArgSet(central_coeff1_num,central_coeff2_num,central_coeff3_num,central_lambda1_num,central_lambda2_num,central_lambda3_num),NTOY);
	...
	myfile.close();
	 */      
	if(channel == 1 and detType == 1)c_pt->SaveAs(Form("/eos/uscms/store/user/tmishra/jetfakepho/Plots%d%s/DoubleEG/JetFakeRate_transfer_DoubleEG_EB_%d%s_ratio.png", RunYear,whichVFP.c_str(),RunYear,whichVFP.c_str()));
	else if(channel == 1 and detType == 2)c_pt->SaveAs(Form("/eos/uscms/store/user/tmishra/jetfakepho/Plots%d%s/DoubleEG/JetFakeRate_transfer_DoubleEG_EE_%d%s_ratio.png", RunYear,whichVFP.c_str(),RunYear,whichVFP.c_str()));
	else if(channel == 2 and detType == 1)c_pt->SaveAs(Form("/eos/uscms/store/user/tmishra/jetfakepho/Plots%d%s/MuonEG/JetFakeRate_transfer_MuonEG_EB_%d%s_ratio.png", RunYear,whichVFP.c_str(),RunYear,whichVFP.c_str()));
	else if(channel == 2 and detType == 2)c_pt->SaveAs(Form("/eos/uscms/store/user/tmishra/jetfakepho/Plots%d%s/MuonEG/JetFakeRate_transfer_MuonEG_EE_%d%s_ratio.png", RunYear,whichVFP.c_str(),RunYear,whichVFP.c_str()));
}
